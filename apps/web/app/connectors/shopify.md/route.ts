...
/**
 * The markdown twin of `/connectors/shopify`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/connectors/shopify");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /connectors/shopify", { status: 500 });
  }
