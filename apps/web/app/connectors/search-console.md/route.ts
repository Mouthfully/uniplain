...
/**
 * The markdown twin of `/connectors/search-console`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/connectors/search-console");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /connectors/search-console", { status: 500 });
  }
