...
/**
 * The markdown twin of `/connectors/woocommerce`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/connectors/woocommerce");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /connectors/woocommerce", { status: 500 });
  }
