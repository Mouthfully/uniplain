...
/**
 * The markdown twin of `/`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /", { status: 500 });
  }
