...
/**
 * The markdown twin of `/privacy`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/privacy");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /privacy", { status: 500 });
  }
