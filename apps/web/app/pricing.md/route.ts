...
/**
 * The markdown twin of `/pricing`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/pricing");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /pricing", { status: 500 });
  }
