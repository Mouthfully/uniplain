...
export const SECURITY_COPY = {
  ...
  metaTitle: "Security",
  metaDescription:
    "What this service enforces in code -- tenant isolation at the row, sealed platform credentials, read access only, and figures that are not training data -- and which assurance documents are not held.",
...
export const CONTROLS: readonly Control[] = [
  {
    ...
    body: [
      ...
      "There is exactly one query in the product that crosses accounts, and it exists so that work can be scheduled. It returns identifiers and times and nothing else -- no figure, no credential, no name -- and its exact column set is asserted rather than described.",
    ],
    check:
  ...
  {
    id: "archive",
    title: "Archived platform responses drop identifiers rather than disguising them",
    body: [
      "When a platform's raw response is kept, it is filtered against a list of fields written for that source. A field nobody listed is dropped, so a platform adding one results in a missing field rather than a leak.",
      "Identifiers are removed, not hashed. An unsalted hash of an email address is reversible by anyone holding a list of email addresses, so storing one would build a pile of pseudonyms that is still personal data and has no reader.",
    ],
    check:
      "The sharp limit, stated rather than implied: fields are removed by name and values are never inspected. Where a person's own words sit inside a field that has to be kept, nothing in that filter protects them.",
  },
  {
    id: "sessions",
    title: "A session is verified rather than believed",
    body: [
      "Every request checks the session against the authentication server instead of trusting what the cookie says. Signing in is a link to an address or an identity provider, and no password is held here to be stolen.",
    ],
    check:
...
export const GAPS: readonly { readonly term: string; readonly note: string }[] = [
  {
    ...
    note: "No retention period is set for any category of data, and nothing deletes on a timer. A request to remove data is handled by hand.",
  },
  {
