...
/**
 * The markdown twin of `/terms`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/terms");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /terms", { status: 500 });
  }
