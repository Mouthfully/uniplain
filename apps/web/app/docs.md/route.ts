...
/**
 * The markdown twin of `/docs`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/docs");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /docs", { status: 500 });
  }
