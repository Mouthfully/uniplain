...
describe("every segment is built on a connector that exists", () => {
  ...
  it("refuses a source that is not implemented", () => {
    // `shopify` STOOD HERE, with a comment calling `/connectors/shopify` "a real page for a
    // connector that does not exist" -- the near-miss this refusal was written for. The connector
    // shipped, so it moved to the other side of the assertion, and `dataforseo_serp` and `impact`
    // carry the case now. Two names rather than one: a single failing example can be special-cased
    // by accident.
    expect(() => assertImplementedSource("dataforseo_serp")).toThrow(/IMPLEMENTED_SOURCE_IDS/);
    expect(() => assertImplementedSource("impact")).toThrow(/IMPLEMENTED_SOURCE_IDS/);
    expect(() => assertImplementedSource("shopify")).not.toThrow();
  });

  /**
   * Platforms an owner in this market would expect and this product cannot read. Naming one here is
   * a promise the visitor tests by connecting it.
   *
   * `Shopify` CAME OFF THIS LIST THE DAY ITS CONNECTOR SHIPPED, and the list is filtered against
   * `IMPLEMENTED_SOURCE_IDS` below so the next one does not have to be remembered. A hand-written
   * list of things that are not true is a list that goes stale in the dangerous direction: it keeps
   * banning a word that has become sayable, and the fix looks like deleting a guard.
   */
  const UNIMPLEMENTED = [
    "Lazada",
    "Shopee",
  ...
  // THE SAFETY NET. If a name above ever becomes an implemented source and nobody removes it, this
  // filter drops it rather than failing on copy that has become true.
  const IMPLEMENTED_LABELS = new Set(
    (IMPLEMENTED_SOURCE_IDS as readonly string[]).map((id) => id.replace(/_/g, "").toLowerCase()),
  );
  const STILL_UNIMPLEMENTED = UNIMPLEMENTED.filter(
    (name) => !IMPLEMENTED_LABELS.has(name.replace(/[^a-z0-9]/gi, "").toLowerCase()),
  );
  ...
  it.each(SEGMENTS.map((segment) => [segment.slug, segment] as const))(
    ...
    (_slug, segment) => {
      ...
      for (const name of STILL_UNIMPLEMENTED) {
