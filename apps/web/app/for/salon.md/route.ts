...
/**
 * The markdown twin of `/for/salon`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/for/salon");
  if (page === undefined) {
    return new Response("registry has no entry for /for/salon", { status: 500 });
  }
