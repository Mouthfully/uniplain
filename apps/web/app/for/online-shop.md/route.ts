...
/**
 * The markdown twin of `/for/online-shop`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/for/online-shop");
  if (page === undefined) {
    return new Response("registry has no entry for /for/online-shop", { status: 500 });
  }
