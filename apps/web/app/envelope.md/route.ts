...
/**
 * The markdown twin of `/envelope`, per the llms.txt convention: same URL, extension replaced.
 *
...
export function GET(): Response {
  const page = AGENT_PAGES.find((p) => p.path === "/envelope");
  if (page === undefined) {
    ...
    return new Response("registry has no entry for /envelope", { status: 500 });
  }
