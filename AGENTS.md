...
| Art. 27 representative | **Required, and absent.** The Art. 27(2) exemption was assessed and is not available | `apps/web/app/_processing/representative-requirement.ts`. Every document said a representative was outstanding and treated it as obvious — **skipping Art. 27(2), which decides whether Art. 27(1) applies at all.** The exemption is closed by the *occasional* limb: a nightly read for the duration of a subscription is the opposite of occasional, by design. (a) is conjunctive, so the one limb that is satisfied (no large-scale special categories) rescues nothing, and the low-risk limb is **deliberately not claimed**. `DESIGNATION_REQUIREMENTS` states the engagement as a specification — Member State (Art. 27(3)), **in writing** (Art. 27(1)), the mandate's scope (Art. 27(4)), and the Art. 30(1) record the representative must maintain, which already exists generated. No mandate is drafted: there is no party to name, and a test bans the drafting phrasings |
